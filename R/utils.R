...
#' @param resolution, Cluster resolution, default NULL attempts to scale based on number of cells.
#' @examples
#' cluster_pipeline(
...
cluster_pipeline <- function(
    seurat, cache_dir, sub_name, protocol, vars_to_regress, parallel_override, cc = TRUE, resolution = NULL
) {
  ...
  if (file.exists(rds)) {
    ...
    if (protocol[4] == "umis") {
      ...
      seurat <- suppressWarnings(
        SCTransform(seurat, vars.to.regress = vars_to_regress, vst.flavor = "v2", verbose = FALSE)
      )
      # ---------------------------------------------------------------------------------
...
gene_plot <- function(subset_list, subset_names, genes) {
  ...
  for (i in 1:length(subset_list)) {
    ...
    if (length(genes) == 2 && all(genes %in% rownames(subset_list[[i]]))) {
      ...
      print(
        FeaturePlot(
          subset_list[[i]], genes, order = TRUE, cols = c("lightgrey", "red"),

---
title: '02 GeneFunnel - `r unlist(strsplit(getwd(), "/"))[7]` - `r unlist(strsplit(getwd(), "/"))[8]`'
author:
...
knit: (function(inputFile, encoding) {
  rmarkdown::render(
    inputFile, encoding = encoding, output_file = file.path(
      ...
      unlist(strsplit(getwd(), "/"))[7], unlist(strsplit(getwd(), "/"))[8], "02_genefunnel.html"
  ))})
---
...
In this document we perform GeneFunnel analysis.
...
packages <- c(
  "conflicted", "Seurat", "GSEABase", "GSVA", "biomaRt", "parallel", "ggplot2", "magrittr", "dplyr", "ggrepel",
  "scuttle", "edgeR", "DT", "scales", "RColorBrewer", "factoextra", "ComplexHeatmap", "reshape2", "pals", "dendextend",
  "GO.db", "tm", "igraph", "qgraph"
)
...
names(metadata_to_plot) <- c("seurat_clusters", "condition", "Sample.ID.Custom", "donor_id")
metadata_to_plot$condition <- "no_label"
metadata_to_plot$Sample.ID.Custom <- "no_label"
...
results_dir <- file.path(
  "..", "..", "..", "..",
  "results", unlist(strsplit(getwd(), "/"))[6], unlist(strsplit(getwd(), "/"))[7], unlist(strsplit(getwd(), "/"))[8]
)
...
seurat$Sample.ID.Custom <- seurat$Sample.ID
seurat$Sample.ID.Custom <- sub("MAP2", "normal", seurat$Sample.ID.Custom)
seurat$Sample.ID.Custom <- sub("AT8", "NFT", seurat$Sample.ID.Custom)
seurat$Sample.ID.Custom <- gsub("-", "_", seurat$Sample.ID.Custom)
...
if (file.exists(rds)) {
  ...
  gene_sets <- getGmt(file.path(assets_dir, "gene-sets", "gprofiler_hsapiens.ENSG", "hsapiens.GO.comb.ENSG.gmt"))
...
if (file.exists(rds) & file.exists(rds2)) {
  ...
  sample1_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_NFT_1")
  sample2_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_NFT_2")
  sample3_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_NFT_3")
  sample4_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_NFT_4")
  sample5_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_NFT_5")
  sample6_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_NFT_6")
  sample7_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_NFT_7")
  sample8_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_NFT_8")

  sample9_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_normal_1")
  sample10_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_normal_2")
  sample11_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_normal_3")
  sample12_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_normal_4")
  sample13_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_normal_5")
  sample14_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_normal_6")
  sample15_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_normal_7")
  sample16_seurat <- subset(seurat, subset = Sample.ID.Custom == "AD_normal_8")

  # Find the minimum number of cells from each of the samples.
  # This will be used to downsample each sample.
  # ----------------------------------------------------------
  downsample <- min(
    dim(sample1_seurat)[2], dim(sample2_seurat)[2], dim(sample3_seurat)[2], dim(sample4_seurat)[2],
    dim(sample5_seurat)[2], dim(sample6_seurat)[2], dim(sample7_seurat)[2], dim(sample8_seurat)[2],
    dim(sample9_seurat)[2], dim(sample10_seurat)[2], dim(sample11_seurat)[2], dim(sample12_seurat)[2],
    dim(sample13_seurat)[2], dim(sample14_seurat)[2], dim(sample15_seurat)[2], dim(sample16_seurat)[2]
  )
  # ----------------------------------------------------------
  ...
  set.seed(1)
  sample4_seurat <- subset(sample4_seurat, cells = sample(Cells(sample4_seurat), downsample))
  set.seed(1)
  sample5_seurat <- subset(sample5_seurat, cells = sample(Cells(sample5_seurat), downsample))
  set.seed(1)
  sample6_seurat <- subset(sample6_seurat, cells = sample(Cells(sample6_seurat), downsample))
  set.seed(1)
  sample7_seurat <- subset(sample7_seurat, cells = sample(Cells(sample7_seurat), downsample))
  set.seed(1)
  sample8_seurat <- subset(sample8_seurat, cells = sample(Cells(sample8_seurat), downsample))
  condition1_seurat <- merge(
    sample1_seurat,
    c(sample2_seurat, sample3_seurat, sample4_seurat, sample5_seurat, sample6_seurat, sample7_seurat, sample8_seurat)
  )

  set.seed(1)
  sample9_seurat <- subset(sample9_seurat, cells = sample(Cells(sample9_seurat), downsample))
  set.seed(1)
  sample10_seurat <- subset(sample10_seurat, cells = sample(Cells(sample10_seurat), downsample))
  set.seed(1)
  sample11_seurat <- subset(sample11_seurat, cells = sample(Cells(sample11_seurat), downsample))
  set.seed(1)
  sample12_seurat <- subset(sample12_seurat, cells = sample(Cells(sample12_seurat), downsample))
  set.seed(1)
  sample13_seurat <- subset(sample13_seurat, cells = sample(Cells(sample13_seurat), downsample))
  set.seed(1)
  sample14_seurat <- subset(sample14_seurat, cells = sample(Cells(sample14_seurat), downsample))
  set.seed(1)
  sample15_seurat <- subset(sample15_seurat, cells = sample(Cells(sample15_seurat), downsample))
  set.seed(1)
  sample16_seurat <- subset(sample16_seurat, cells = sample(Cells(sample16_seurat), downsample))
  condition2_seurat <- merge(
    sample9_seurat,
    c(
      sample10_seurat, sample11_seurat, sample12_seurat, sample13_seurat,
      sample14_seurat, sample15_seurat, sample16_seurat
    )
  )
  ...
  rm(
    ...
    sample9_seurat, sample10_seurat, sample11_seurat, sample12_seurat,
    sample13_seurat, sample14_seurat, sample15_seurat, sample16_seurat
  )
  seurat <- seurat[rowSums(as.matrix(GetAssayData(seurat, slot = "counts")) > 0) > 0, ]

  # keep <- c()
  # for (sample in unique(seurat$Sample.ID.Custom)) {
  #   sub_seurat <- subset(seurat, subset = Sample.ID.Custom == sample)
  #   keep <- unique(
  #     append(keep, names(which(rowSums(as.matrix(GetAssayData(sub_seurat, slot = "counts")) > 0) > downsample / 25)))
  #   )
  # }
  # seurat <- seurat[keep, ]

  add <- unique(unlist(geneIds(gene_sets))[which(unlist(geneIds(gene_sets)) %notin% rownames(seurat))])
  new_mat <- GetAssayData(seurat, "counts")
  null_mat <- matrix(0, length(add), ncol(new_mat))
  for (i in seq(nrow(null_mat))) {
    set.seed(i)
    null_mat[i, sample(ncol(null_mat), 1)] <- 1
  }
  ...
  gene_anno <- getBM(attributes, "ensembl_gene_id", rownames(seurat), mart)
  gene_anno <- gene_anno[gene_anno$ensembl_gene_id %in% unique(unlist(geneIds(gene_sets))), ]
  add <- which(gene_anno$external_gene_name == "")
  for (i in add) {
    gene_anno[i, 1] <- gene_anno[i, 2]
  }

  tmp <- lapply(geneIds(gene_sets), function(x) x[!(x %notin% gene_anno$ensembl_gene_id)])
  for (i in seq(length(gene_sets@.Data))) {
    suppressWarnings(gene_sets[[i]]@geneIds <- tmp[[i]])
  }
  tmp <- filterGeneSets(gene_sets, 5, 50)
  gene_sets <- gene_sets[names(gene_sets) %in% names(tmp)]

  seurat <- seurat[rownames(seurat) %in% gene_anno$ensembl_gene_id, ]

  seurat$condition <- factor(seurat$SORT)
  levels(seurat$condition)[levels(seurat$condition) == "AT8"] <- "BA9_EX_Neuron_NFT"
  levels(seurat$condition)[levels(seurat$condition) == "MAP2"] <- "BA9_EX_Neuron_normal"
...
seurat_expanded <- seurat
sub_name <- "sctransform"
seurat <- cluster_pipeline(seurat, cache_dir, sub_name, protocol, vars_to_regress, parallel_override, FALSE)
seurat
...
add <- which(rownames(seurat_expanded) %notin% rownames(seurat_old))
new_mat <- GetAssayData(seurat_old, "counts")
null_mat <- GetAssayData(seurat_expanded)[add, ]
new_mat <- rbind(new_mat, null_mat)
seurat <- CreateSeuratObject(new_mat, meta.data = seurat_old[[]])

new_mat <- GetAssayData(seurat_old)
min <- min(new_mat[new_mat > 0])
null_mat[null_mat == 1] <- min
new_mat <- rbind(new_mat, null_mat)
seurat <- SetAssayData(seurat, new.data = new_mat)

rm(seurat_old)

hist(
  as.matrix(GetAssayData(seurat)), main = "SCTransform normalized log counts distribution after adding missing genes"
)
...
rds2 <- file.path(cache_dir, "pseudobulk_filtered.rds")
if (file.exists(rds)) {
  sce <- readRDS(rds)
  sce_filt <- readRDS(rds2)
} else {
  sce <- as.SingleCellExperiment(seurat)
  sce <- suppressWarnings(
    aggregateAcrossCells(
      sce, colData(sce)[ , c("condition", "donor_id")], use_exprs_values = "counts", statistics = "sum"
    )
  )
  keep <- filterByExpr(counts(sce), group = sce$condition, min.total.count = 50, min.count = 50)
  sce_filt <- sce[keep, ]
...
if (file.exists(rds)) {
  ...
  gsva <- gsva(logcounts(sce), gene_sets, method = "ssgsea", ssgsea.norm = FALSE, verbose = FALSE)
...
# --------------------------------------
design <- model.matrix(~ 0 + sce$condition + sce$donor_id)
colnames(design) <- c(
  levels(sce$condition), "Subject2", "Subject3", "Subject4", "Subject5", "Subject6", "Subject7", "Subject8"
)
fit <- lmFit(gsva, design)
contrast_mat <- makeContrasts(BA9_EX_Neuron_NFT-BA9_EX_Neuron_normal, levels = design)
...
names(gsva_list_pseudobulk) <- c(
  "All DE", paste0("Upregulated in ", levels(sce$condition)[1]),
  paste0("Downregulated in ", levels(sce$condition)[2])
)
# --------------------------------------
...
# -------------------------------------------------
tests <- decideTests(cont_fit, p.value = 1e-4)
tests_up <- tests[tests[ , 1] == 1, ]
tests_down <- tests[tests[ , 1] == -1, ]
results <- topTable(cont_fit, number = Inf)
...
# ----------------------------------------------------
tests <- decideTests(cont_fit, p.value = 1e-5)
tests_up <- tests[tests[ , 1] == 1, ]
tests_down <- tests[tests[ , 1] == -1, ]
results <- topTable(cont_fit, number = Inf)
...
**Upregulated in NFTs GSE Initial**
...
**Upregulated in NFTs GSE Strict**
...
**Upregulated in NFTs GSE Stricter**
...
**Upregulated in NFTs GSE Strictest**
...
**Downregulated in NFTs GSE Initial**
...
**Downregulated in NFTs GSE Strict**
...
**Downregulated in NFTs GSE Stricter**
...
**Downregulated in NFTs GSE Strictest**
...
# --------------------------------
design <- model.matrix(~ 0 + sce_filt$condition + sce_filt$donor_id)
colnames(design) <- c(
  levels(sce_filt$condition), "Subject2", "Subject3", "Subject4", "Subject5", "Subject6", "Subject7", "Subject8"
)
...
names(markers_list_pseudobulk) <- c(
  "All DE", paste0("Upregulated in ", levels(sce_filt$condition)[1]),
  paste0("Downregulated in ", levels(sce_filt$condition)[2])
)
# --------------------------------------
...
gene_anno <- gene_anno[order(match(gene_anno$ensembl_gene_id, rownames(new_mat))), ]
...
**Upregulated in NFTs DEG Initial**
...
**Upregulated in NFTs DEG Strict**
...
**Upregulated in NFTs DEG Stricter**
...
**Upregulated in NFTs DEG Strictest**
...
**Downregulated in NFTs DEG Initial**
...
**Downregulated in NFTs DEG Strict**
...
**Downregulated in NFTs DEG Stricter**
...
**Downregulated in NFTs DEG Strictest**
...
ggplot(data, aes(x, y)) +
  ...
  labs(x = "Number of DEGs", y = "Number of DEGs")
```
...
for (j in seq(length(gene_sets_sub_up@.Data))) {
  if (
    length(which(gene_sets_sub_up[[j]]@geneIds %in% markers_list_pseudobulk_all[[2]])) >=
      round(length(gene_sets_sub_up[[j]]@geneIds) / 4)
    ) {
...
for (j in seq(length(gene_sets_sub_down@.Data))) {
  if (
    length(which(gene_sets_sub_down[[j]]@geneIds %in% markers_list_pseudobulk_all[[3]])) >=
      round(length(gene_sets_sub_down[[j]]@geneIds) / 4)
  ) {
...
anno <- HeatmapAnnotation(
  ...
  cell_type = anno_block(
    ...
    labels = c(levels(sce$condition)[[1]], levels(sce$condition)[[2]]),
    labels_gp = gpar(col = "black", fontsize = 14)
  ),
  col = list(
    donor = c(
      "Subject1" = anno_color[1], "Subject2" = anno_color[2], "Subject3" = anno_color[3], "Subject4" = anno_color[4],
      "Subject5" = anno_color[5], "Subject6" = anno_color[6], "Subject7" = anno_color[7], "Subject8" = anno_color[8]
    )
  ),
...
draw(
  Heatmap(
    ...
    row_names_max_width = max_text_width(rownames(mat), gpar(fontsize = 12)),
    ...
    row_names_gp = gpar(fontsize = 12, col = unlist(row_colors)),
...
layout_up <- qgraph.layout.fruchtermanreingold(
  ...
  area = vcount(graph_up), repulse.rad = vcount(graph_up)
)
...
layout_down <- qgraph.layout.fruchtermanreingold(
  ...
  area = vcount(graph_down), repulse.rad = vcount(graph_down)
)
...
layout <- qgraph.layout.fruchtermanreingold(
  ...
  area = vcount(graph), repulse.rad = vcount(graph)
)
...
png(file.path(results_dir, "02_genefunnel_network.png"), 150, 100, "in", res = 72)
...
plot(
    ...
    edge.width = (E(graph)$weight * 1.25) ^ 3,
    vertex.shape = "pie",
    ...
    vertex.label.cex = ifelse(DE == "yes", ((all_DE_match$Freq) * 2), 2),
...
legend(
    "right",
    legend = paste0(
      "Module ", unique(hub_gene_data$module), ": ", gsub("\n", " ", top_words[unique(hub_gene_data$module)])
    ),
    pch = 20,
...
color2 <- colorRampPalette(rev(brewer.pal(11, "RdYlBu")))(100)
```

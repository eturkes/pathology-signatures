...
if (file.exists(rds) & file.exists(rds2)) {
  ...
  remove <- which(gene_anno$external_gene_name == "")
  gene_anno <- gene_anno[-remove, ]
  gene_anno <- gene_anno[gene_anno$ensembl_gene_id %in% unique(unlist(geneIds(gene_sets))), ]

  tmp <- lapply(geneIds(gene_sets), function(x, y) na.omit(fastmatch::fmatch(x, y)), gene_anno$ensembl_gene_id)
...
if (file.exists(rds)) {
  ...
  keep <- filterByExpr(counts(sce), group = sce$condition, min.total.count = 10)
  sce <- sce[keep, ]
...
seurat_symbols <- SetAssayData(seurat_symbols, new.data = log2(as.matrix(GetAssayData(seurat_symbols, "counts")) + 1))
seurat_symbols <- SetAssayData(seurat_symbols, "scale.data", t(scale(t(GetAssayData(seurat_symbols)), FALSE)))
# -------------------------------------------
```
...
for (j in seq(length(gene_sets_sub_up@.Data))) {
  if (
    length(which(gene_sets_sub_up[[j]]@geneIds %in% markers_list_pseudobulk_all[[2]])) >=
      round(length(gene_sets_sub_up[[j]]@geneIds) / 3)
    ) {
...
for (j in seq(length(gene_sets_sub_down@.Data))) {
  if (
    length(which(gene_sets_sub_down[[j]]@geneIds %in% markers_list_pseudobulk_all[[3]])) >=
      round(length(gene_sets_sub_down[[j]]@geneIds) / 3)
  ) {
...
png(file.path(results_dir, "02_genefunnel_network.png"), 450, 300, "in", res = 72)
...
plot(
    ...
    vertex.label.cex = ifelse(DE == "yes", ((all_DE_match$Freq + 1) * 10), 20),
